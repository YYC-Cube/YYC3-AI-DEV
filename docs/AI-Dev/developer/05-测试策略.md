...
> 当前基线: **312 tests / 26 文件 · 全绿**（2026-08-19 实测）

## 一、测试框架配置

### 1.1 Vitest 配置

项目根 `vitest.config.ts`：
...
├─────────────────────────────────────────┤
│         组件测试 (Phase 3)               │  ← React Testing Library
├─────────────────────────────────────────┤
│         引擎单元测试 (Phase 1) ✅         │  ← Vitest
├─────────────────────────────────────────┤
│         类型检查 (持续) ✅               │  ← tsc --noEmit
└─────────────────────────────────────────┘
```

### 2.1 当前测试基线（2026-08-19 实测）

| 插件包 | 测试文件 | 测试数量 | 状态 |
| -------- | ---------- | ---------- | ------ |
| plugin-target | target-engine.test.ts + engine-integration.test.ts | 35 | ✅ 全绿 |
...
| plugin-dynasty | 6 test files | 38 | ✅ 全绿 |
| plugin-llm | crypto/router/providers/sse/key-manager + boundaries（依赖边界守卫） | 61 | ✅ 全绿 |
| shell | 8 test files（EventBus/Storage/组件/Hub/LLMBridge） | 74 | ✅ 全绿 |
| family-agents | family-agents.test.ts | 10 | ✅ 全绿 |
| family-core | ecosystem.test.ts | 8 | ✅ 全绿 |
| dynasty-core | protocol/bus/honors 边界守卫 | 8 | ✅ 全绿 |
| **合计** | **28 文件** | **324** | ✅ |
...
| 全量测试 | `npx vitest run` | 312/312 全绿 |
...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| v1.0.0 | 2026-08-19 | 重建测试策略，更新基线至 312 tests / 26 文件 | YanYuCloudCube Team |
