...
> **文档定位**：开发者实施向导（怎么接插件、怎么用 Shell API）。架构契约与变量词库/路由接口的权威定义见 [ARCHITECTURE.md](../ARCHITECTURE.md)，两文互补不重复。
...
- 该边界由 [`boundaries.test.ts`](../../../packages/plugin-llm/src/__tests__/boundaries.test.ts) 自动守护
...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| v1.1.0 | 2026-09-18 | 插件包数校准 13→12（shell 独立于 plugin-* 计数）；补文档定位声明与 ARCHITECTURE 交叉链接 | YanYuCloudCube Team |
| v1.0.0 | 2026-08-19 | 重建架构总纲，更新至 324 tests 基线 | YanYuCloudCube Team |
