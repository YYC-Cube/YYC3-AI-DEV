...
import { DYNASTY_AGENTS, DYNASTY_HUB_COMMANDS } from "@yyc3/plugin-dynasty";
...
const CARD_COLORS = { monitor: "#00d4ff", ops: "#FF6600", ai: "#AA55FF", "ai-family": "#00FF88", dev: "#E8E8E8", admin: "#FFDD00", dynasty: "#C9A96E" };
...
const SYSTEMS = [
  ...
  { id: "dynasty", name: "王朝治理", desc: "三省六部制", icon: Crown, path: "/dynasty" },
  { id: "dev", name: "开发工具", desc: "设计/终端/IDE", icon: Code2, path: "/dev" },
  { id: "admin", name: "系统管理", desc: "设置/用户/安全", icon: ShieldCheck, path: "/admin" },
];

const ALL_CMDS = [
  ...DYNASTY_HUB_COMMANDS.map(cmd => ({ ...cmd, action: () => { window.location.hash = "#/dynasty"; } })),
...
const familySummary = `${FAMILY_PERSONAS.length} 位家人 · ${DYNASTY_AGENTS.length} 位朝臣 · 7 系统协同`;
  ...
  const router = createHashRouter([
    {
      element: <Layout><Outlet /></Layout>, children: [
        ...
        { path: "dynasty", lazy: async () => ({ Component: () => null as any }) },
  ...
  return (
    <div className="p-8 text-center" style={{ paddingTop: "8vh" }}>
      ...
      <div className="mt-12 flex justify-center gap-6 text-[rgba(0,212,255,0.15)] text-xs">
        ...
        <span>{DYNASTY_AGENTS.length} 位朝臣</span><span>·</span>
        <span>7 子系统</span>
      </div>
    </div>
  );
