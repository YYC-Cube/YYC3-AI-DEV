...
describe("EventBus 跨插件通信", () => {
  ...
  describe("Dynasty 命名空间 (dynasty:*)", () => {
    it("dynasty 朝臣激活事件应跨系统传播", () => {
      const receiver = vi.fn();
      bus.on("dynasty:activate", receiver);

      bus.emit("dynasty:activate", { dynastyId: "qin", agentId: "shangyang" });

      expect(receiver).toHaveBeenCalledWith({ dynastyId: "qin", agentId: "shangyang" });
    });
  });
